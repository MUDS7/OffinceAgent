...
export const DOCUMENT_EXTENSIONS = ["txt", "md", "csv", "json", "pdf", "xlsx", "xls", "docx"];
export const BINARY_PREVIEW_EXTENSIONS = new Set(["pdf", "xlsx", "xls", "docx"]);
...
export const EXCEL_AGENT_SYSTEM_PROMPT = [
  ...
  "The JSON must be strict JSON: double-quoted keys/strings, no trailing commas, and no raw line breaks inside string values. Use separate args.paragraphs/items entries instead of embedding multiline strings.",
  "Required JSON shape:",
  "{",
...
export const DOCX_AGENT_SYSTEM_PROMPT = [
  ...
  '  "command": "replace_text" | "delete_text" | "replace_paragraph" | "insert_paragraph" | "append_paragraph" | "insert_table" | "insert_blocks",',
  '  "args": { "command-specific arguments": "values" },',
  '  "message": "short user-facing Chinese message"',
  "}",
  "Use action=docx_execute only when the requested file change is clear and maps to one supported command.",
  "One insert_paragraph or append_paragraph command can insert multiple paragraphs by using args.paragraphs as an ordered array of strings.",
  "Use insert_blocks when the content to insert mixes text and uploaded-document images. In args.items, use {\"type\":\"paragraph\",\"text\":\"...\"} for text and {\"type\":\"image\",\"image_ref\":\"...\"} for referenced images.",
  ...
  "For insert_paragraph use args.text for one paragraph, or args.paragraphs for multiple paragraphs. Use args.position of before, after, start, or end. Use args.block_index or args.target_text unless position is start/end.",
  "For append_paragraph use args.text for one paragraph, or args.paragraphs for multiple paragraphs.",
  "When the uploaded-document reference contains a parent section plus child sections, and the user asks to supplement/copy that section, include the child-section headings and their content as separate paragraphs in args.paragraphs instead of reducing them to one summary paragraph.",
  "When uploaded-document reference content includes image_ref entries, preserve those images at the matching position with insert_blocks instead of writing only an [IMAGE:...] placeholder.",
